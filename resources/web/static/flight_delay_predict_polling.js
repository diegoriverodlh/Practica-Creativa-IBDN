...
function conditionalPoll(data) {
  try {
    ...
    // Intenta hacer el parseo manualmente, pero con control
    var response;
    try {
      response = JSON.parse(responseText);
    } catch (e) {
      throw new Error("La respuesta no es JSON: " + responseText);
    }
    ...
    if (response.status === "OK") {
      ...
      $("#result").empty().append("❌ Estado inesperado del servidor.");
    }

  } catch (error) {
    console.error("❌ Error al parsear JSON:", error);
    $("#result").empty().append("❌ Error del servidor. La respuesta no es válida.");
  }
}
